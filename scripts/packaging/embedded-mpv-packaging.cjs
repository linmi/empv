...
const macosAudioOutputBackendMarker = Buffer.from('avfoundation', 'utf8')
...
function hasRequiredMacosAudioOutputBackend(addonPath) {
  return (
    fs.existsSync(addonPath) && fs.readFileSync(addonPath).includes(macosAudioOutputBackendMarker)
  )
}

function hasRequiredMacosAudioRuntimePolicy(manifest) {
  const mesonFlags = manifest?.mpv?.mesonFlags
  return (
    Array.isArray(mesonFlags) &&
    mesonFlags.includes('-Davfoundation=enabled') &&
    mesonFlags.includes('-Dcoreaudio=disabled')
  )
}
...
function toPortableSymlinkTarget(sourcePath, destinationPath) {
  ...
  if (path.dirname(resolvedTarget) === path.dirname(sourcePath)) {
    return path.basename(resolvedTarget)
  }

  return path.relative(path.dirname(destinationPath), resolvedTarget)
}

function copySymlink(sourcePath, destinationPath) {
  ensureDir(path.dirname(destinationPath))
  fs.rmSync(destinationPath, { force: true })
  fs.symlinkSync(toPortableSymlinkTarget(sourcePath, destinationPath), destinationPath)
}

function copyFileOrSymlink(sourcePath, destinationPath) {
  if (fs.lstatSync(sourcePath).isSymbolicLink()) {
    copySymlink(sourcePath, destinationPath)
    return
  }
...
function copyDirectoryPreserveSymlinks(sourceDir, destinationDir, filter) {
  ...
  for (const entry of fs.readdirSync(sourceDir, { withFileTypes: true })) {
    ...
    if (entry.isDirectory()) {
      copyDirectoryPreserveSymlinks(sourcePath, destinationPath, filter)
      continue
    }
...
function listRuntimeEntries(directoryPath) {
  if (!fs.existsSync(directoryPath)) {
    return []
  }
...
function listRuntimeSymlinks(directoryPath) {
  if (!fs.existsSync(directoryPath)) {
    return []
  }
...
function isSystemDependency(dependencyPath) {
  return systemRuntimePathPrefixes.some((prefix) => dependencyPath.startsWith(prefix))
}

function isForbiddenDependency(dependencyPath) {
  return forbiddenRuntimePathPrefixes.some((prefix) => dependencyPath.startsWith(prefix))
}

function resolvePathToken(tokenPath, binaryPath) {
  if (tokenPath.startsWith('@loader_path/')) {
    return path.resolve(path.dirname(binaryPath), tokenPath.replace('@loader_path/', ''))
  }
...
function patchDylibIds(libDir) {
  for (const dylibPath of listRuntimeFiles(libDir)) {
    ...
    run('install_name_tool', ['-id', `@loader_path/${path.basename(dylibPath)}`, dylibPath])
  }
}

function patchBinaryDependencies(binaryPath, dependencyBaseDir, replacementPrefix) {
  const availableRuntimeFiles = new Set(
    listRuntimeEntries(dependencyBaseDir).map((runtimePath) => path.basename(runtimePath))
  )
  ...
  for (const dependencyPath of parseOtoolDependencies(binaryPath)) {
    ...
    run('install_name_tool', ['-change', dependencyPath, replacementPath, binaryPath])
  }
}
...
function copyRuntimeToNativeBuild({ runtimeLibDir, outputLibDir, runtimeOrigin, runtimeManifest }) {
  ...
  for (const dylibPath of dylibs) {
    copyFileOrSymlink(dylibPath, path.join(outputLibDir, path.basename(dylibPath)))
  }

  if (runtimeOrigin === 'vendored-lgpl') {
    for (const runtimePath of listRuntimeSymlinks(runtimeLibDir)) {
      copyFileOrSymlink(runtimePath, path.join(outputLibDir, path.basename(runtimePath)))
    }
  }
  ...
  if (!fs.existsSync(path.join(outputLibDir, 'libmpv.2.dylib'))) {
    ...
    if (path.basename(copiedLibMpvPath) !== 'libmpv.2.dylib') {
      copyFileOrSymlink(copiedLibMpvPath, path.join(outputLibDir, 'libmpv.2.dylib'))
    }
  }
  ...
  const manifest = {
    ...
    libDir: 'lib',
    runtimeFiles: listRuntimeEntries(outputLibDir).map((runtimePath) => path.basename(runtimePath)),
    dylibs: listDylibs(outputLibDir).map((dylibPath) => path.basename(dylibPath))
  }

  fs.writeFileSync(
    path.join(path.dirname(outputLibDir), 'runtime-manifest.json'),
  ...
  for (const binaryPath of binaryPaths) {
    ...
    for (const dependencyPath of parseOtoolDependencies(binaryPath)) {
      if (isForbiddenDependency(dependencyPath)) {
        errors.push(`${binaryPath} links to forbidden runtime path: ${dependencyPath}`)
      }
      ...
      if (dependencyPath.startsWith('@loader_path/')) {
        ...
        if (!fs.existsSync(resolvedPath)) {
          errors.push(`${binaryPath} links to missing bundled dependency: ${dependencyPath}`)
        }
      }

      if (!isSystemDependency(dependencyPath) && !dependencyPath.startsWith('@loader_path/')) {
        errors.push(`${binaryPath} links to non-bundled runtime path: ${dependencyPath}`)
      }
    }
  }
...
function getPackagedRuntimeCandidates(libDir, platform, nativeDir) {
  switch (platform) {
    case 'darwin':
      return [path.join(libDir, 'libmpv.2.dylib'), path.join(libDir, 'libmpv.dylib')]
    case 'win32':
      return [
...
function validatePackagedEmbeddedMpv(resourceDir, options = {}) {
  ...
  const runtimeCandidates = getPackagedRuntimeCandidates(libDir, platform, unpackedNativeDir)
  const runtimeDirectory = platform === 'win32' ? unpackedNativeDir : libDir
  if (
    ...
    errors.push(
      [
        `Missing bundled embedded MPV runtime for ${platform} in ${runtimeDirectory}.`,
        'Expected one of:',
  ...
  if (platform === 'darwin') {
    ...
    if (
      ...
      !hasRequiredMacosAudioRuntimePolicy(JSON.parse(fs.readFileSync(manifestPath, 'utf8')))
    ) {
      errors.push(
        `Embedded MPV runtime must enable AVFoundation and exclude CoreAudio: ${manifestPath}`
      )
    }

    if (process.platform !== 'darwin') {
      errors.push('macOS embedded MPV link validation must run on a macOS host.')
    } else if (!commandExists('otool')) {
      errors.push('otool is required to validate embedded MPV macOS packaging.')
    } else {
