...
// The session API every runtime backend exports. This surface is loaded only in
// the isolated playback process. In particular, the macOS CALayer presenter is
// not part of this addon: keeping it in a separate libmpv-free binary is the
// boundary that prevents dyld from loading libmpv into Electron main.
...
// macOS runtime half: configures the Mach sender before the first session.
export type LibMpvLayerRuntimeAddon = LibMpvEmbeddedCoreAddon & {
  getPresentationKind(): 'layer'
  configureFrameLink(serviceName: string): void
}

// macOS main-process half: a separate N-API binary containing only the AppKit /
// CALayer presenter, Mach receiver and window observation bridge. It must not
// link libmpv or export any playback/session method.
export type LibMpvLayerPresenterAddon = {
...
export type LibMpvEmbeddedNativeAddon = LibMpvLayerRuntimeAddon | LibMpvWindowAddon

export type LoadedEmbeddedLibMpvAddon =
  | {
      presentationKind: 'layer'
      addon: LibMpvLayerRuntimeAddon
...
export type LoadedEmbeddedLibMpvPresenterAddon = {
  addon: LibMpvLayerPresenterAddon
  runtime: LibMpvRuntime
}
...
function hasAnyFunction(candidate: Record<string, unknown>, names: readonly string[]): boolean {
...
const LAYER_RUNTIME_FUNCTIONS = ['configureFrameLink'] as const

const LAYER_PRESENTER_FUNCTIONS = [
  'getPresenterKind',
  'createPresenter',
  'setPresenterBounds',
...
function isLibMpvLayerRuntimeAddon(
  candidate: Record<string, unknown>
): candidate is Record<string, unknown> & LibMpvLayerRuntimeAddon {
  return hasFunctions(candidate, CORE_FUNCTIONS) && hasFunctions(candidate, LAYER_RUNTIME_FUNCTIONS)
}

function isLibMpvLayerPresenterAddon(
  candidate: Record<string, unknown>
): candidate is Record<string, unknown> & LibMpvLayerPresenterAddon {
  return (
    hasFunctions(candidate, LAYER_PRESENTER_FUNCTIONS) && candidate.getPresenterKind() === 'layer'
  )
}
...
export type NormalizedEmbeddedAddon =
  | { presentationKind: 'layer'; addon: LibMpvLayerRuntimeAddon }
...
export function normalizeEmbeddedAddon(value: unknown): NormalizedEmbeddedAddon {
  ...
  if (!isObject(candidate) || !hasFunctions(candidate, CORE_FUNCTIONS)) {
    throw new Error('libmpv runtime addon must export the embedded MPV session API.')
  }

  const presentationKind = candidate.getPresentationKind()

  if (presentationKind === 'layer') {
    if (!isLibMpvLayerRuntimeAddon(candidate)) {
      throw new Error('libmpv runtime addon reports layer but is missing its Mach sender API.')
    }
    if (hasAnyFunction(candidate, LAYER_PRESENTER_FUNCTIONS)) {
      throw new Error(
        'libmpv runtime addon must not export the macOS presenter bridge; load empv_presenter.node in Electron main.'
      )
    }
...
export function normalizeEmbeddedPresenterAddon(value: unknown): LibMpvLayerPresenterAddon {
  const candidate = isObject(value) && 'default' in value ? value.default : value
  if (!isObject(candidate) || !hasFunctions(candidate, LAYER_PRESENTER_FUNCTIONS)) {
    throw new Error('empv presenter addon must export the macOS layer presenter bridge API.')
  }
  if (hasAnyFunction(candidate, CORE_FUNCTIONS)) {
    throw new Error('empv presenter addon must not export or load the libmpv session API.')
  }
  const kind = candidate.getPresenterKind()
  if (kind !== 'layer') {
    throw new Error(`empv presenter addon reported an unknown presenter kind: ${String(kind)}.`)
  }
  if (!isLibMpvLayerPresenterAddon(candidate)) {
    throw new Error('empv presenter addon failed its layer presenter contract.')
  }
  return candidate
}
...
export async function loadEmbeddedLibMpvPresenterAddon(
  options: EmbeddedLibMpvAddonLoadOptions = {}
): Promise<LoadedEmbeddedLibMpvPresenterAddon> {
  const runtime = await assertLibMpvRuntime(options)
  if (!runtime.presenterAddonPath) {
    throw new Error(
      `The resolved libmpv runtime at ${runtime.runtimeDirectory} has no macOS presenter addon.`
    )
  }
  const requireAddon = options.requireAddon ?? createRequire(import.meta.url)
  const addon = normalizeEmbeddedPresenterAddon(requireAddon(runtime.presenterAddonPath))
